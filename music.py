...
class MusicCog(commands.Cog):
    ...
    async def queue_task(self, ctx: commands.Context, voice: discord.VoiceClient):
        ...
        while True:
            ...
            if queue["playing"]["random"]:
                queue_keys = [str(x) for x in queue.keys() if x != "playing"]
            ...
            req_by = client.get_user(int(tgt_queue["req_by"]))
            voice.play(discord.FFmpegPCMAudio(tgt_url, before_options=get_youtube.before_args))
            voice.source = discord.PCMVolumeTransformer(voice.source)
            voice.source.volume = vol
    ...
    @commands.command(name="재생", description="음악을 재생합니다.", aliases=["play", "p", "ㅔ"])
    ...
    @commands.command(name='루프', description="재생중인 음악을 무한 반복하거나 무한 반복을 해제합니다.", aliases=["무한반복", "loop", "repeat", "u"])
    async def music_loop(self, ctx):
        ...
        if queue["playing"]["loop"] is not True:
            ...
            res = await confirm.confirm(self.client, ctx, msg)
            if res is not True:
    ...
    @commands.command(name="정지", description="음악 재생을 멈춥니다.", aliases=["stop", "ㄴ새ㅔ", "st"])
    ...
    @commands.command(name="일시정지", description="음악을 일시정지합니다.", aliases=["pause", "ps", "ㅔㄴ", "i"])
    ...
    @commands.command(name="대기리스트", description="현재 대기 리스트를 보여줍니다.", aliases=["q", "ㅂ", "대기열", "재생리스트", "pl", "ql", "queuelist", "playlist", "비", "ㅔㅣ"])
    async def queue_list(self, ctx):
        ...
        temp_ql_embed.set_author(name=ctx.guild.name)
        ...
        one_embed.set_author(name=ctx.guild.name, icon_url=ctx.guild.icon.url)
        one_embed.set_image(url=playing_thumb)
        req_by = self.client.get_user(int(queue_list["playing"]["req_by"]))
        ...
        one_embed.add_field(name="대기중인 음악 개수", value=f"{len([x for x in queue_list.keys() if x != 'playing'])}개")
        ...
        for x in queue_list.keys():
            ...
            queue_req_by = self.client.get_user(int(queue_list[x]["req_by"]))
        ...
        next_req_by = self.client.get_user(int(next_song["req_by"]))
        ...
        next_embed.set_author(name=ctx.guild.name, icon_url=ctx.guild.icon.url)
        next_embed.set_image(url=next_thumb)
        embed_list.append(ql_embed)
        embed_list.append(next_embed)
        await page.start_page(self.client, ctx, embed_list, embed=True)
